...
export function RequestCard({ data, onConnect, currentUserId, className }: RequestCardProps) {
  ...
  return (
    <article
      className={cn(
        'group relative p-5 sm:p-6 rounded-3xl border border-border/80 bg-card/85 dark:bg-card/75 backdrop-blur-xl shadow-xs hover:shadow-lg hover:border-primary/40 transition-all duration-300 flex flex-col justify-between gap-4',
        className
      )}
    >
      ...
      <div className="flex items-start sm:items-center justify-between gap-3 flex-wrap sm:flex-nowrap">
        ...
        <div className="flex items-center gap-3 min-w-0">
          <Avatar className="h-10 w-10 sm:h-11 sm:w-11 ring-2 ring-border shrink-0 shadow-2xs">
      ...
      <div className="pt-3 border-t border-border/60 flex items-center justify-between gap-3">
        ...
        {isOwner ? (
          <Badge
            variant="outline"
            className="rounded-xl text-xs font-semibold px-3 py-1.5 bg-secondary/60 text-muted-foreground border-border/80"
          >
            Your Active Pitch
          </Badge>
        ) : (
          <Button
            ...
            className="btn-gradient-primary text-white rounded-xl text-xs font-bold shadow-xs px-4 touch-target min-h-[38px] transition-all active:scale-95"
          >
