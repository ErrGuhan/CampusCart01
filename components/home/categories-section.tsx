        ...
        {categories.map((category) => {
          ...
          return (
            <Link
              ...
              className={cn(
                'group flex flex-col h-full items-center justify-between gap-3 rounded-3xl border border-border/80 bg-card/85 dark:bg-card/75 backdrop-blur-md p-4 sm:p-5 text-center transition-all duration-300 hover:border-primary/50 hover:shadow-lg hover:-translate-y-1 active:scale-95 shadow-xs',
                isEmpty && 'opacity-60 grayscale hover:grayscale-0 hover:opacity-100 border-dashed bg-card/60'
              )}
            >
              <div className={cn(
                'flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-2xl bg-primary/10 text-primary dark:bg-primary/20 transition-all group-hover:bg-gradient-to-br group-hover:from-primary group-hover:to-indigo-600 group-hover:text-white group-hover:scale-105 shadow-xs',
