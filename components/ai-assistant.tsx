        ...
        <div className="fixed bottom-20 sm:bottom-24 left-2.5 right-2.5 sm:left-auto sm:right-6 z-50 sm:w-[390px] h-[74vh] max-h-[560px] rounded-[32px] border border-white/80 dark:border-border/60 bg-gradient-to-b from-[#F8F3FF] via-[#FAF7FF] to-[#FFF5EE] dark:from-[#171026] dark:via-[#130D20] dark:to-[#1B1120] backdrop-blur-2xl shadow-2xl flex flex-col overflow-hidden animate-fade-in-up">
          ...
          <div className="p-3.5 sm:p-4 border-b border-white/60 dark:border-border/60 bg-white/70 dark:bg-card/70 backdrop-blur-md flex items-center justify-between">
            ...
            <button
              ...
              className="h-8 w-8 rounded-2xl bg-white/80 dark:bg-card/80 border border-white/80 dark:border-border shadow-2xs flex items-center justify-center text-muted-foreground hover:text-foreground"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs scrollbar-thin">
            ...
            {messages.map((m, idx) => {
              ...
              return (
                <div key={idx} className={cn('flex items-end gap-2', isAi ? 'justify-start' : 'justify-end')}>
                  ...
                  {isAi && (
                    <div className="h-7 w-7 rounded-full bg-gradient-to-tr from-pink-400 via-purple-500 to-indigo-400 shadow-2xs flex items-center justify-center text-white shrink-0 mb-1 ring-2 ring-purple-300/30">
                      ...
                      <div className="mt-2 space-y-1.5 w-full">
                        {m.matchedProducts.map((p) => (
                          <Link
                            ...
                            onClick={() => setOpen(false)}
                            className="flex items-center gap-2.5 p-2 rounded-2xl border border-white/80 dark:border-border/80 bg-white/90 dark:bg-card/90 shadow-2xs hover:border-purple-400/50 transition-all"
                          >
            ...
            {loading && (
              <div className="flex items-center gap-2 text-xs text-muted-foreground bg-white/80 dark:bg-card/80 p-2.5 rounded-2xl w-fit border border-white/80 shadow-2xs">
          ...
          {/* Quick Prompt Chips */}
          <div className="p-2 border-t border-white/60 dark:border-border/60 flex items-center gap-1.5 overflow-x-auto bg-white/50 dark:bg-card/50 scrollbar-none">
            {quickPrompts.map((prompt) => (
              <button
                key={prompt}
                onClick={() => handleSend(prompt)}
                className="rounded-xl border border-white/80 dark:border-border bg-white/80 dark:bg-card/80 px-2.5 py-1 text-[10px] font-semibold text-muted-foreground hover:text-foreground hover:border-purple-400/40 whitespace-nowrap transition-colors shadow-2xs"
              >
                {prompt}
              </button>
            ))}
