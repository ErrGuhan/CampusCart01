  ...
  async function handleSend(queryText: string) {
    ...
    setTimeout(() => {
      ...
      if (matchedProds.length > 0 || matchedGigs.length > 0) {
        ...
        replyText = `I couldn't find any listings currently matching "${queryText}". Would you like to post a request on the campus board so other students can fulfill it?`;
    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-36 md:bottom-24 right-5 z-40 w-[92vw] sm:w-[400px] h-[520px] rounded-3xl border border-border bg-background/95 backdrop-blur-xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-5">
                    ...
                    <div className="mt-2 space-y-1.5 w-full">
                      {m.matchedProducts.map((p) => (
                        <Link
                          ...
                          onClick={() => setOpen(false)}
                          className="flex items-center gap-2.5 p-2 rounded-xl border border-border bg-card hover:border-primary/40 transition-colors"
                        >
                          <img
                            src={p.images[0]}
                            alt={p.name}
          ...
          {/* Quick Prompt Chips */}
          <div className="p-2 border-t border-border/60 flex items-center gap-1.5 overflow-x-auto bg-card/40">
