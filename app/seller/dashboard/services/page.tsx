  ...
  async function handleSave() {
    if (!user) {
      toast({
        ...
        description: 'Please sign in to offer freelance services.',
        variant: 'destructive',
      });
      return;
    }
    ...
    setSaving(true);
    try {
      const sellerDisplayName = profile?.display_name || user.displayName || user.email?.split('@')[0] || 'Campus Freelancer';
      const sellerUsername = profile?.username || user.email?.split('@')[0] || 'freelancer';
